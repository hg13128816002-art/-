...
test("server-renders the finished Synesthesia Canvas shell", async () => {
  ...
  assert.match(html, /D 半音五声音阶/);
  assert.match(html, /E 紧张五声音阶/);
  ...
  const drumNames = ["活力鼓组", "深层重鼓", "像素鼓机", "故障切片鼓", "霓虹派对鼓"];
...
test("ships the audio, canvas, export, sharing, and responsive product code", async () => {
  ...
  assert.match(page, /limiter\.ratio\.value = 20/);
  assert.match(page, /SIDECHAIN_ATTACK_SECONDS/);
  assert.match(page, /cancelAndHoldAtTime\(attackStart\)/);
  assert.match(page, /linearRampToValueAtTime\(Math\.max\(EPSILON, depth\), duckTime\)/);
  ...
  assert.match(page, /scheduledEventIds: Map<number, Set<string>>/);
  assert.match(page, /state\.events = prepareTransportEvents/);
  assert.match(page, /projectEndStep\(next\)/);
  assert.match(page, /schedulerPumpRef\.current\?\.\(\)/);
  assert.match(page, /!scheduledInCycle\.has\(event\.shape\.id\)/);
  assert.match(page, /scheduledInCycle\.add\(event\.shape\.id\)/);
  ...
  assert.match(page, /DRAW_INTENT_THRESHOLD_PX/);
  assert.match(page, /hasStarted: false/);
  assert.match(page, /distance < DRAW_INTENT_THRESHOLD_PX/);
  assert.match(page, /interaction\.kind === "draw" && !interaction\.hasStarted/);
  assert.match(page, /点按画单个音符；拖动画出连续音符/);
  assert.match(page, /onPointerCancel=\{onCanvasPointerCancel\}/);
  assert.match(page, /onLostPointerCapture=\{onCanvasPointerCancel\}/);
  assert.doesNotMatch(page, /canvasPointerGuardUntilRef/);
  ...
  assert.doesNotMatch(page, /gestureTime \+ 240/);
  ...
  assert.match(page, /E 暗色五声音阶/);
  assert.doesNotMatch(page, /日系|日式|和风|樱花|三味线|太鼓|祭典|平调子|阴音阶|阴旋|岩户|阳音阶/);
